...
series, backtests compounded return, and evaluates confluence filters
(RSI, MACD, ATR%) to rank signal quality and suppress fake crossovers.

FIXES vs v1:
  - Open trades at end of data are now closed at last bar price
  - Cumulative return is compounded (not arithmetic sum)
  - Sharpe uses simple mean/std (no invalid per-trade annualisation)
  - Dead score variable removed; confluence score stored in trade log
  - top5 filtered to pairs with actual trades
  - valid_from uses max(sma, ema*2)+1 for proper warmup
────────────────────────────────────────────────────────────────────────────
"""
...
def _backtest_pair(
    ...
    Returns a dict with compounded performance statistics.
    """
    close = df["Close"]
    ema_s = _ema(close, ema_period)
    sma_s = _sma(close, sma_period)

    # FIX: proper warmup for both EMA and SMA
    valid_from = max(sma_period, ema_period * 2) + 1
    ...
    for i in range(valid_from, len(close)):
        ...
        if golden and not in_trade:
            ...
            if ok:
                ...
                entry_idx   = i
                # FIX: store confluence score in the trade
                conf_score = sum([
                    1 if (pd.notna(rsi_v) and 50 <= rsi_v <= 65) else 0,
                    1 if (pd.notna(mac_v) and mac_v > 0)          else 0,
                    1 if (pd.notna(vol_v) and vol_v >= 1.3)        else 0,
                    1 if (pd.notna(atr_v) and atr_v < 2.0)         else 0,
                ])
            else:
                filtered_out += 1

        elif death and in_trade:
            _close_trade(trades, df, close, entry_price, entry_idx, i, conf_score if 'conf_score' in dir() else 0)
            in_trade    = False
            entry_price = np.nan
            entry_idx   = None

    # FIX: close any open trade at last bar
    if in_trade and entry_idx is not None:
        _close_trade(trades, df, close, entry_price, entry_idx, len(close) - 1,
                     conf_score if 'conf_score' in dir() else 0, open_trade=True)

    return _compute_stats(trades, ema_period, sma_period, raw_signals, filtered_out)


def _close_trade(trades, df, close, entry_price, entry_idx, exit_idx,
                 conf_score=0, open_trade=False):
    ...
    trades.append({
        ...
        "exit_date":    df.index[exit_idx].strftime("%d-%b-%y"),
        "conf_score":   conf_score,
        "open_trade":   open_trade,
    })


def _compute_stats(trades, ema_period, sma_period, raw_signals, filtered_out) -> dict:
    empty = {
    ...
    # FIX: compounded cumulative return
    cum_ret = (np.prod([(1 + r / 100) for r in rets]) - 1) * 100
    ...
    # FIX: simple Sharpe (mean / std) — per-trade returns, no false annualisation
    mean_r = np.mean(rets)
    std_r  = np.std(rets, ddof=1) if len(rets) > 1 else 0.01
...
def compute_crossover_matrix(
    ...
    # FIX: top5 only from pairs that actually generated trades
    valid = [m for m in matrix if m["trades"] > 0]
    best  = valid[0] if valid else matrix[0]
    top5  = valid[:5]
    ...
    suggestions = _build_suggestions(last_rsi, last_macd, last_atr, last_vol)
...
def _build_suggestions(rsi, macd, atr, vol) -> list[str]:
    s = []
    if rsi is not None:
        if rsi > 70:
            s.append(f"RSI({rsi}) is OVERBOUGHT — avoid new long entries; wait for pullback to 45–60 zone.")
        elif rsi < 40:
            s.append(f"RSI({rsi}) is OVERSOLD — crossover may be a dead-cat bounce; require 2 consecutive closes above SMA.")
        else:
            s.append(f"RSI({rsi}) is in MOMENTUM ZONE (40–70) — crossover signals carry higher conviction here.")

    if macd is not None:
        if macd > 0:
            s.append(f"MACD histogram is POSITIVE ({macd}) — momentum is aligned; adds confluence to any golden cross signal.")
        else:
            s.append(f"MACD histogram is NEGATIVE ({macd}) — bearish momentum undercurrent; treat EMA crossover with caution.")

    if atr is not None:
        if atr > 3.5:
            s.append(f"ATR% = {atr}% — HIGH VOLATILITY. Widen stop-loss to 2× ATR below entry; reduce position size by 30–50%.")
        elif atr < 1.0:
            s.append(f"ATR% = {atr}% — LOW VOLATILITY compression. Breakout may be imminent; watch for volume expansion on crossover day.")
        else:
            s.append(f"ATR% = {atr}% — NORMAL RANGE. Standard 1.5× ATR stop-loss is appropriate.")

    if vol is not None:
        if vol >= 1.5:
            s.append(f"Volume ratio = {vol}× avg — STRONG INSTITUTIONAL PARTICIPATION. High-conviction signal.")
        elif vol < 0.8:
            s.append(f"Volume ratio = {vol}× avg — WEAK VOLUME. Crossover lacks institutional support; reduce size or wait.")
        else:
            s.append(f"Volume ratio = {vol}× avg — MODERATE. Prefer ≥ 1.3× for highest-conviction entries.")
    ...
    s.append(
        ...
        "(3) Avoid entries within 3 days of results/event."
    )
    return s
